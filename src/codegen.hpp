...
//
// `is_function_boundary` is true for the body_scope of a user-defined
// function. lookup() walks the full chain (so a function can read the
// hoisted globals on its parent), but has() stops at the boundary, so
// `v = ...` inside a function body declares a fresh local instead of
// being read as "reassign the hoisted global v" -- the same Python-like
// shadow rule the interpreter now uses.
struct CScope {
...
public:
    ...
    std::string generate(const std::vector<StmtPtr>& program,
        ...
        // Hoist top-level constants to file-scope so function bodies can
        // see them. We walk every top-level Assign and infer a type
        // structurally; everything we recognize gets a `static TYPE
        // NAME;` decl up top and is pre-registered in global_scope, so
        // emit_stmt's reassign branch fires (no shadowing declarator
        // inside main()). Anything we don't recognize falls through to
        // the old behavior: main()-local, invisible to functions.
        std::vector<std::pair<std::string, CType>> hoisted_globals;
        for (const auto& s : program) {
            if (s->kind != StmtKind::Assign) continue;
            if (s->target) continue;            // indexed write: not a global decl
            if (!s->expr) continue;
            // Re-scan a name we've already hoisted: keep the same type.
            if (global_scope.has_local(s->name)) continue;
            // Don't shadow a stdlib / user function name: that would emit
            // both `static double mean;` and `double mean(knot_vec)` and
            // the C compiler rejects the dual declaration. The script
            // reassigning a name that's also a function is rare; when it
            // happens, fall back to the old main-local behavior.
            if (fn_sigs_.count(s->name)) continue;
            CType t = infer_top_level_type(*s->expr);
            if (t == CType::Unknown) continue;
            global_scope.define(s->name, t);
            hoisted_globals.push_back({s->name, t});
        }
        ...
        // Emit file-scope decls for hoisted top-level globals.
        std::ostringstream globals_decls;
        for (const auto& g : hoisted_globals) {
            globals_decls << "static " << ctype_name(g.second) << " " << g.first << ";\n";
        }
...
private:
    ...
    ExprResult emit_call(const Expr& e, CScope* scope) {
        ...
        if (name == "pow")  { if (args.size() != 2 || args[0].type != CType::Num || args[1].type != CType::Num) fail(e.span, "pow(num, num)"); return {join("pow"), CType::Num}; }
        if (name == "floor"){ if (args.size() != 1 || args[0].type != CType::Num) fail(e.span, "floor(num)"); return {join("floor"), CType::Num}; }
        if (name == "ceil") { if (args.size() != 1 || args[0].type != CType::Num) fail(e.span, "ceil(num)");  return {join("ceil"),  CType::Num}; }
        if (name == "tan")  { if (args.size() != 1 || args[0].type != CType::Num) fail(e.span, "tan(num)");   return {join("tan"),   CType::Num}; }
        if (name == "asin") { if (args.size() != 1 || args[0].type != CType::Num) fail(e.span, "asin(num)");  return {join("asin"),  CType::Num}; }
        if (name == "acos") { if (args.size() != 1 || args[0].type != CType::Num) fail(e.span, "acos(num)");  return {join("acos"),  CType::Num}; }
        if (name == "atan") { if (args.size() != 1 || args[0].type != CType::Num) fail(e.span, "atan(num)");  return {join("atan"),  CType::Num}; }
        if (name == "atan2"){ if (args.size() != 2 || args[0].type != CType::Num || args[1].type != CType::Num) fail(e.span, "atan2(num, num)"); return {join("atan2"), CType::Num}; }
    ...
    void emit_stmt(const Stmt& s, CScope* scope,
        ...
        switch (s.kind) {
            case StmtKind::Assign: {
                ...
                indent(body, depth);
                // Inside a function, an assignment to a name that only
                // exists in an outer scope creates a fresh local
                // (Python-style shadow), so a stdlib helper writing
                // `v = abs(...)` doesn't trip over a script's
                // `v = [10, 20, 30]` global. has() stops at function
                // boundaries; block sub-scopes still walk up to the
    ...
    void emit_fn(const Stmt& s, CScope* global_scope) {
        ...
        body_scope.is_function_boundary = true;
        for (size_t i = 0; i < s.params.size(); ++i) {
    ...
    // Structurally infer the type of a top-level Assign RHS, without
    // emitting code (so this is safe to call before the main emission
    // pass). Used by the global-hoist pass in compile(). Returns
    // CType::Unknown if the structure isn't one we want to commit to.
    CType infer_top_level_type(const Expr& e) {
        switch (e.kind) {
            case ExprKind::NumberLit: return CType::Num;
            case ExprKind::BoolLit:   return CType::Bool;
            case ExprKind::StringLit: return CType::Str;
            case ExprKind::VecLit:    return CType::Vec;
            case ExprKind::MatLit:    return CType::Mat;
            case ExprKind::Unary:
                if (e.unop == UnOp::Neg && e.rhs)
                    return infer_top_level_type(*e.rhs);
                return CType::Unknown;
            case ExprKind::Binary: {
                CType lt = e.lhs ? infer_top_level_type(*e.lhs) : CType::Unknown;
                CType rt = e.rhs ? infer_top_level_type(*e.rhs) : CType::Unknown;
                if (lt == CType::Num && rt == CType::Num) return CType::Num;
                return CType::Unknown;
            }
            case ExprKind::Call: {
                ...
                if (fn == "linspace" || fn == "arange" || fn == "fill")
                    return CType::Vec;
                if (fn == "sqrt" || fn == "exp" || fn == "log"
                 || fn == "sin"  || fn == "cos" || fn == "tan"
                 || fn == "abs"  || fn == "pow" || fn == "floor"
                 || fn == "ceil" || fn == "len" || fn == "rows"
                 || fn == "cols")
                    return CType::Num;
                return CType::Unknown;
            }
